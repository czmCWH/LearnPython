...
#   - 数据清洗（正则表达式处理）：
#     年份：如 (1997)，需去掉括号
#     上映时间：如 2026-05-08 (US)，需去掉 (US) 部分，并去掉多余空格
#     时长：如 1h 25m，需转换为分钟
#
...
MOVIE_LIST_FILE = "movie_page5_1.csv"
...
def get_movie_release_dates(movie_release_dates):
  ...
  #  movie_release_date.split(" ")[0]
  match = re.search(r"\d{4}-\d{2}-\d{2}", movie_release_date)
...
def get_movie_durations(movie_durations):
  ...
  h_match = re.search(r"(\d+)h", movie_duration)
  hours = int(h_match.group(1)) * 60 if h_match else 0    # ⚠️⚠️⚠️，.group(1) 表示匹配到的第一个括号内的内容。
  m_match = re.search(r"(\d+)m", movie_duration)
